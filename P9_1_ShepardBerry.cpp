...
*/
void __declspec (naked) asmChangeOddToEven(short[], short, short&) {
    __asm {
            ...
            sub edx, edx        // init odd counter to 0
    ...
    TOP:    mov eax, [ebx]      // move current value to scratch
            ...
            inc dx              // increment odd counter
    NEXT:   add ebx, 2          // move to next value in the array
            loop TOP            // decrement CDctr and jump to TOP if CDctr != 0

            mov eax, [ebp + 16] // mov odd counter into scratch
            mov [eax], dx       // mov odd count into memory
...
int main() {
    ...
    // PROCESS
    asmChangeOddToEven(arr, length, oddCount);
    ...
    cout << endl;
    cout << "And there were " << oddCount << " odd values" << endl;
